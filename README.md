...
2. 调用多重IP定位服务解析地理位置
3. 优先获取城市级信息
4. 直接调用心知天气API：`https://weather.seniverse.com/?token=...`
5. 返回精准的天气信息
6. 每小时重新定位，适应用户位置变化

### 🗺️ 智能定位示例

系统现在通过心知天气API提供精准的天气信息：
...
**特点**：直接使用心知天气的专业气象数据，无需手动配置城市

### 🏙️ 智能城市识别

系统通过IP定位自动识别用户所在城市：

- **自动识别**: 基于用户IP地址自动确定所在城市
- **全球覆盖**: 支持全球任意IP地址定位
- **精准匹配**: 直接调用心知天气API获取对应城市的专业气象数据
- **智能降级**: 当无法精确定位时，自动降级到城市级定位

**无需手动配置**：系统会根据用户的实际位置自动选择最合适的城市数据
